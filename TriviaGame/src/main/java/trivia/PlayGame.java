...
public class PlayGame {
    ...
    public static void main(String[] args) {
      ...
      if (!((Game) aGame).isPlayable()){
          System.err.println("This game is not playable");
      }

        for (int i = 1; i <= playerCount; i++) {
            System.out.print("Player " + i + " name: ");
            String playerName = scanner.nextLine();
            boolean res =  aGame.addPlayer(playerName);
            while (!res){
                System.err.println("Recommencez, un autre joueur à le même nom");
                System.out.print("Player " + i + " name: ");
